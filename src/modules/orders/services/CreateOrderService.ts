...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if(!existentProducts.length){
      throw new AppError('did not find products');
    }
    ...
    if(checkInexistentProducts.length){
      throw new AppError(`did not find products ${checkInexistentProducts[0].id}`);

    }

    const findProductsWihtnoQuantityAvailable = products.filter(
      product => existentProducts.filter(
        p=>p.id === product.id)[0].quantity < product.quantity
    );

    if(findProductsWihtnoQuantityAvailable.length){
      throw new AppError(`quantity not available`);
    }

    const serialProducts = products.map(product => ({
